...
void Spell::SelectImplicitCasterDestTargets(SpellEffIndex effIndex, SpellImplicitTargetInfo const& targetType)
{
    ...
    switch (targetType.GetTarget())
    {
        ...
        case TARGET_DEST_CASTER_FRONT_LEAP:
        {
            ...
            ground = map->GetHeight(phasemask, x, y, z);
            ...
            if (!m_caster->HasUnitMovementFlag(MOVEMENTFLAG_FALLING) || (z - ground < 25.0f))
            {
                ...
                //}
                dest = SpellDestination(destx, desty, destz + 0.5f, orientation);
            }
            else
            {
                bool col = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(mapid, x, y, z + 0.5f, destx, desty, z + 0.5f, destx, desty, z, -0.5f);
                // check dynamic collision
                bool dcol = m_caster->GetMap()->getObjectHitPos(phasemask, x, y, z + 0.5f, destx, desty, z + 0.5f, destx, desty, z, -0.5f);

                // collision occured
                if (col || dcol)
                {
                    // move back a bit
                    destx = destx - (0.6 * cos(orientation));
                    desty = desty - (0.6 * sin(orientation));
                }
